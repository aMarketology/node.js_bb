...
class BlackBookWallet {
   ...
   * 
   * @param {string|object} config - Configuration object or L1 URL for backward compatibility
   * @param {string} supabaseUrl - Supabase project URL for auth/vault storage (legacy)
   * @param {string} supabaseKey - Supabase anon key (legacy)
   */
  constructor(config = 'http://localhost:8080', supabaseUrl = null, supabaseKey = null) {
    // Support both new config object and legacy URL string
    if (typeof config === 'string') {
      this.apiUrl = config;
      this.supabaseUrl = supabaseUrl;
      this.supabaseKey = supabaseKey;
    } else {
      this.apiUrl = config.apiUrl || config.l1Url || 'http://localhost:8080';
      this.supabaseUrl = config.supabaseUrl;
      this.supabaseKey = config.supabaseKey;
      
      // Pre-set credentials if provided in config
      if (config.address) this.address = config.address;
      if (config.publicKey) this.publicKey = hexToBytes(config.publicKey);
      if (config.privateKey) this.privateKey = hexToBytes(config.privateKey);
  ...
  /**
   * Send tokens (alias for transferSimple - production compatibility)
   */
  async sendTokens(to, amount) {
    return this.transferSimple(to, amount);
  }
  ...
  /**
   * Connect test account (production-ready alias for initFromTestAccount)
   * 
   * @param {string} accountName - 'alice', 'bob', or 'dealer'
   * @returns {Promise<object>} - Wallet info { address, publicKey, balance }
   */
  async connectTestAccount(accountName) {
    const result = this.initFromTestAccount(accountName);
    
    // Check server health and get current balance
    try {
      const balance = await this.getBalance();
      result.balance = balance;
      result.connected = true;
      result.server = this.apiUrl;
    } catch (error) {
      result.balance = null;
  ...
  // ═══════════════════════════════════════════════════════════════
  // L2 SESSION MANAGEMENT - Production Features
  // ═══════════════════════════════════════════════════════════════

  /**
   * Open L2 session for fast trading
   */
  async openL2Session(params) {
    if (!this.address) throw new Error('Wallet not initialized');
    
    const { amount, l2Url } = params;
    
    const response = await fetch(`${this.apiUrl}/l2/session/open`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        l1Address: this.address,
        amount,
        l2Url: l2Url || 'http://localhost:1234',
    ...
    return {
      ...
      amount: amount,
      l2Address: result.l2Address || result.l2_address,
      message: result.message
    };
  }

  /**
   * Get active L2 sessions
   */
  async getL2Sessions() {
    if (!this.address) throw new Error('Wallet not initialized');
    
    const response = await fetch(`${this.apiUrl}/l2/sessions/${this.address}`);
    const result = await response.json();
  ...
  /**
   * Settle L2 session and return funds to L1
   */
  async settleL2Session(params) {
    if (!this.address) throw new Error('Wallet not initialized');
    
    const { sessionId } = params;
    
    const response = await fetch(`${this.apiUrl}/l2/session/settle`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        sessionId,
    ...
    return {
      success: result.success || response.ok,
      settledAmount: result.settledAmount || result.settled_amount,
      pnl: result.pnl,
      message: result.message
    };
  }
...
// Helper function for SDK creation
function createBlackBookSDK(config) {
  return new BlackBookWallet(config);
}
